...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const removeRoleBtn = document.querySelector('.remove-role');

  addRoleBtn.addEventListener('click', function () {
      const newIndex = roleContainer.children.length + 1;
      const newRoleItem = `
          <div class="mb-3 role-item">
              <label for="selectRole${newIndex}" class="form-label">Role</label>
              <select name="selectRole[]" id="selectRole${newIndex}" class="form-control">
      ...
      `;
      roleContainer.insertAdjacentHTML('beforeend', newRoleItem);
  });

  removeRoleBtn.addEventListener('click', function () {
      if (roleContainer.children.length > 1) {
          roleContainer.lastElementChild.remove();
      }
  });
});